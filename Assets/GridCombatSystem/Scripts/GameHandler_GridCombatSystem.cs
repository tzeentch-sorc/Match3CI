...
public class GameHandler_GridCombatSystem : MonoBehaviour {
    ...
    private void Start() {
		combatSystem = GridCombatSystem.Instance;

		movementTilemap.SetTilemapVisual(movementTilemapVisual);
        
        foreach (UnitGridCombat unitGridCombat in combatSystem.unitGridCombatArray)
        {
			var unitPointOnGrid = GridPathfinding.instance.GetConvertedVectorPosition(unitGridCombat.GetPosition());
			GridPathfinding.instance.SetWalkable(unitPointOnGrid.x, unitPointOnGrid.y, false);
		}
